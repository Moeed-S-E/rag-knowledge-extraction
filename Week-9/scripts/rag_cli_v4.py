...
def main(
    ...
    topic: str = typer.Option(None, "--topic", "-t", help="Filter retrieval by specific topic cluster"),
    entity_boost: str = typer.Option(None, "--entity-boost", "-e", help="Boost ranking of chunks containing this specific entity (e.g., 'Federal Reserve')")
):
    """Start an interactive chat with Entity-Boosted RAG."""
    header = "[bold green]Welcome to the RAG CLI v4 (Entity-Boosted)[/bold green]"
    if topic:
        header += f"\n[bold magenta]Filtering by Topic: {topic}[/bold magenta]"
    if entity_boost:
        header += f"\n[bold magenta]Boosting by Entity: {entity_boost}[/bold magenta]"
    ...
    while True:
        ...
        # Over-fetch if boosting
        fetch_k = k * 3 if entity_boost else k
        ...
        if not retrieved_docs:
            ...
            # -----------------------------------------------------------------
            # Custom Re-ranking Logic (Entity Boosting)
            # -----------------------------------------------------------------
            if entity_boost:
                boosted_results = []
                for i in range(len(retrieved_docs)):
                    ...
                    # Boost logic: if entity is inside the comma-separated string
                    # Lower distance is better in Chroma (L2 or Cosine distance)
                    if entity_boost.lower() in ent_str.lower():
                        dist -= 0.5  # Artificial boost
                ...
                # Sort by new adjusted distances (ascending)
                boosted_results = sorted(boosted_results, key=lambda x: x["dist"])
                
                # Keep top K
                top_results = boosted_results[:k]
                
                retrieved_docs = [r["doc"] for r in top_results]
                retrieved_ids = [r["id"] for r in top_results]
                retrieved_metadatas = [r["meta"] for r in top_results]

            chunks_fmt = []
            for i, doc in enumerate(retrieved_docs):
                t_label = retrieved_metadatas[i].get("topic", "Unknown") if retrieved_metadatas and retrieved_metadatas[i] else "Unknown"
                e_label = retrieved_metadatas[i].get("entities", "") if retrieved_metadatas and retrieved_metadatas[i] else ""
                chunks_fmt.append(f"--- Chunk {retrieved_ids[i]} [Topic: {t_label}] [Entities: {e_label}] ---\n{doc}")
            context = "\n\n".join(chunks_fmt)
